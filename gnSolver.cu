...
namespace solver{
    ...
    void buildNLSKernel(gnSolver& solver,
        ...
        int cols = sensor.cols;
        int rows = sensor.rows;
        int size = rows * cols;
        ...
        for (int idx = index; idx < size; idx += stride) {
            ...
            float3 s_n = make_float3(sourceCloud.normals[idx].x, sourceCloud.normals[idx].y, sourceCloud.normals[idx].y);

            // check for valid data in depth and normal from source image
            if (source_depth[idx] < DEPTH_MIN || source_depth[idx] > DEPTH_MAX || isnan(s_n.x) || isnan(s_n.y) || isnan(s_n.z) || s_n==make_float3(0.0f, 0.0f, 0.0f)) continue;   // todo...  hard-coded limit for depth!!!
            ...
            float3 q = getPoint3d(idx, target_depth[index], sensor);  // make sure transform is correct
            // printf("%d, %f \n", idx, cosTheta);
    ...
    void gnSolver::solveSparceLinearSystem(void){
        // // // --- Initialize cuSPARSE
        ...
        // warpField.updateNodesDQ(result);

        // // write result into file for debug
        // #ifdef DEBUG
        //     writeMatrixToTxt("../logs/result.txt", result, sizeof(Jacobian), N, N, 1, 0, N);
        // #endif  
    ...
    void gnSolver::solve(Eigen::Matrix4f& pose, 
        ...
        for(int pylvl = 0; pylvl < 3; pylvl++) {
            ...
            for (iterationNum = 1 ; iterationNum < OptProp.lvlIterationNum[pylvl]; iterationNum++) {
                ...
                if (change.norm() <= OptProp.minIncrement) break;
                prev_increment = increment;
            }
